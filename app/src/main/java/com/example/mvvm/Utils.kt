...
object Utils {
    ...
    fun formatPriceSymbol(s: String?): String? {
        return if (s == null) "Tk 0" else "Tk " + formatPriceComma(s)
    }


    fun formatPriceSymbol(s: Double): String {
        return "Tk " + formatPriceComma(s)
    }
...
//    }
    ...
    fun isTimeInside(startTime: String, endTime: String, currentTime: String): Boolean {
        return try {
            val mStartTime = fromStringToCalenderDate(startTime)
            val mEndTime = fromStringToCalenderDate(endTime)
            val mCurrentTime = fromStringToCalenderDate(currentTime)
            mCurrentTime!!.after(mStartTime) && mCurrentTime.before(mEndTime)
    ...
    private fun fromStringToCalenderDate(time: String): Calendar? {
        val mData =  if (time.contains("T")){
            time.split("T")[1]
        }else{
            time
        }
        val timeData = SimpleDateFormat("HH:mm:ss").parse(mData)
        val calendar = Calendar.getInstance()
    ...
    fun isUTCDateTimeAfterCurrentDateTime(dateTime: String): Boolean {
        val dateTimeData: Date? =
            SimpleDateFormat("yyyy-MM-dd'T'HH:mm:ss.SSSZ", Locale.ENGLISH).parse(dateTime)
        val currentDateTime =
            SimpleDateFormat("yyyy-MM-dd'T'HH:mm:ss.SSSZ", Locale.ENGLISH).format(Date())
        val currentDateTimeData =
            SimpleDateFormat("yyyy-MM-dd'T'HH:mm:ss.SSSZ", Locale.ENGLISH).parse(currentDateTime)
        return dateTimeData!!.after(currentDateTimeData) ?: false
    }

    fun isUTCDateTimeBeforeCurrentDateTime(dateTime: String): Boolean {
        val dateTimeData: Date? =
            SimpleDateFormat("yyyy-MM-dd'T'HH:mm:ss.SSSZ", Locale.ENGLISH).parse(dateTime)
        val currentDateTime =
            SimpleDateFormat("yyyy-MM-dd'T'HH:mm:ss.SSSZ", Locale.ENGLISH).format(Date())
        val currentDateTimeData =
            SimpleDateFormat("yyyy-MM-dd'T'HH:mm:ss.SSSZ", Locale.ENGLISH).parse(currentDateTime)
        return dateTimeData?.before(currentDateTimeData) ?: false
    }

    fun isCurrentDateTimeInside(startTime: String, endTime: String): Boolean {
        return isUTCDateTimeBeforeCurrentDateTime(startTime) && isUTCDateTimeAfterCurrentDateTime(endTime)
    }
    ...
    fun goToQInAppStore(mContext: Context) {
